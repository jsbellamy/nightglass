...
test.describe("rendered-output evidence seam", () => {
  ...
  test("evidence: cross-webview-delivery / evidence: dock-surfaces — Management Dock populates from the Battle Tile over a shared bus and cycles Armory, Character, and Stage with Character sub-tab scenes", async ({
    ...
    for (const subTab of subTabs) {
      await dock.click(`[data-character-sub-tab="${subTab}"]`);
      ...
      const clipState = await dock.evaluate((activeSubTab) => {
        ...
        if (activeSubTab === "talents") {
          ...
          if (scroll) {
            ...
            // Scroll host may extend to the panel edge; only fail hard horizontal clip.
      ...
      await dock.screenshot({ path: `${SCREENSHOTS}/04b-character-${subTab}.png` });
    }
  ...
  test("evidence: dock-navigation-ownership — Armory and Character share one left rail; Stage has zero rail width; no compact Armory selector", async ({
    ...
    async function measureRail(tab: "armory" | "character" | "stage") {
      ...
      return dock.evaluate(() => {
        ...
        return {
          // The prior false-positive only checked HTMLElement.hidden while
          // `.character-picker { display: flex; }` kept layout width.
    ...
    await dock.screenshot({ path: `${SCREENSHOTS}/05-dock-navigation-ownership-stage.png` });
    await context.close();
  });
