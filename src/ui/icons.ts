...
import pinpointShotUrl from "../assets/icons/pinpoint-shot.png";
import plumweaveAegisUrl from "../assets/icons/plumweave-aegis.png";
import pommelBreakUrl from "../assets/icons/pommel-break.png";
...
import sweepingArcUrl from "../assets/icons/sweeping-arc.png";
...
export const ICON_SOURCES = {
  ...
  "pinpoint-shot": { url: pinpointShotUrl, width: CONTENT_ICON_SIZE, height: CONTENT_ICON_SIZE },
  "plumweave-aegis": { url: plumweaveAegisUrl, width: CONTENT_ICON_SIZE, height: CONTENT_ICON_SIZE },
  "pommel-break": { url: pommelBreakUrl, width: CONTENT_ICON_SIZE, height: CONTENT_ICON_SIZE },
  ...
  "sweeping-arc": { url: sweepingArcUrl, width: CONTENT_ICON_SIZE, height: CONTENT_ICON_SIZE },
...
export function collectContentTalentIconKeys(content: Content): string[] {
  ...
  for (const classKit of content.classes) {
    for (const tier of [classKit.talents, ...(classKit.talentTiers ?? [])]) {
  ...
  for (const ability of content.abilities) {
    if (
      (ability.slot === "basic" || ability.slot === "core" || ability.slot === "talent") &&
      ability.iconKey
    ) {
...
export function collectSelectableAbilityIconKeys(
  content: Pick<Content, "classes" | "abilities">,
): { abilityId: string; iconKey: string | undefined }[] {
  const abilityById = new Map(content.abilities.map((ability) => [ability.id, ability]));
  const entries: { abilityId: string; iconKey: string | undefined }[] = [];

  for (const classKit of content.classes) {
    const basic = abilityById.get(classKit.basicAbilityId);
    if (basic) {
      entries.push({ abilityId: basic.id, iconKey: basic.iconKey });
    }
    for (const coreId of classKit.coreAbilityIds) {
      const core = abilityById.get(coreId);
      if (core) {
        entries.push({ abilityId: core.id, iconKey: core.iconKey });
      }
    }
    for (const tier of [classKit.talents, ...(classKit.talentTiers ?? [])]) {
      for (const abilityId of tier.abilityRow) {
        ...
        if (ability) {
          entries.push({ abilityId: ability.id, iconKey: ability.iconKey });
        }
      }
    }
  }
