...
| Dock surfaces (Armory → Character → Stage tabs on one row; each surface populated; Character **Build** then **Stats** at **800×480** with recovered **Variant C** board — Character header owns Level/Talent Points; simultaneous Loadout + Talent columns on **Build**; full left Character rail; no outer Character scroll) | `evidence: dock-surfaces` | Same suite; orchestrator reviews harness-emitted `e2e-screenshots/` after `npm run test:evidence` (top-level `04-dock-*` plus Character **Build** / **Stats** review scenes at 800×480) |
