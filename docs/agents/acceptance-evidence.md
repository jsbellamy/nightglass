...
| Equipment icon content tier (collection cards) | `evidence: equipment-icon-content-tier` | `e2e/rendered-evidence.spec.ts` via `npm run test:evidence`; committed legibility artifact at `docs/research/evidence/124-equipment-icon-consumers/armory-slot-strip.png` |
| Equipment icon chrome tier (Armory slot strip legibility) | `evidence: equipment-icon-chrome-legibility` | Same scenario as content tier in `e2e/rendered-evidence.spec.ts`; artifact path above |
