...
| Equipment icon chrome tier (no chrome consumer remains after Armory strip deletion; Armory worn strip content-tier icons carry the slug with an explicit tier change note) | `evidence: equipment-icon-chrome-legibility` | Same scenario in `e2e/rendered-evidence.spec.ts`; committed review artifact at `docs/research/evidence/124-equipment-icon-consumers/armory-worn-strip.png` |
